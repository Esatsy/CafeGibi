...
namespace CafeGibi
{
    public partial class frmMasalar : Form
    {
        ...
        private void frmMasalar_Load(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT DURUM,ID FROM dbo.masalar", con);
            ...
            while (dr.Read())
            {
                foreach (Control item in this.Controls)
                {
                    if (item is Button)
                    {
                        ...
                        {
                            item.BackgroundImage = (System.Drawing.Image)(Properties.Resources.Adsız_tasarım__12_);
                        }
                        else if (item.Name == "btnMasa" + dr["ID"].ToString() && dr["DURUM"].ToString() == "2")
                        {
                            ...
                            item.BackgroundImage = (System.Drawing.Image)(Properties.Resources.Adsız_tasarım__13_);
                        }
                        else if (item.Name == "btnMasa" + dr["ID"].ToString() && dr["DURUM"].ToString() == "3")
                        {
                            item.BackgroundImage = (System.Drawing.Image)(Properties.Resources.Adsız_tasarım__15_);
                        }
                        else if (item.Name == "btnMasa" + dr["ID"].ToString() && dr["DURUM"].ToString() == "4")
                        {
                            item.BackgroundImage = (System.Drawing.Image)(Properties.Resources.Adsız_tasarım__14_);
                        }
